...
public final class FirstPartTasks {
    ...
    private static IntStream getRatingsStream(Album album) {
        return album.getTracks().stream().mapToInt(Track::getRating);
    }
    ...
    public static List<String> allTracksSorted(Stream<Album> albums) {
        return albums.map(Album::getTracks).flatMap(Collection::stream)
    ...
    public static List<Album> sortedFavorites(Stream<Album> s) {
        return s.filter(album -> getRatingsStream(album).max().orElse(0) > 95)
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(Comparator.comparingInt(album -> getRatingsStream(album).max().orElse(0)));
    }

    // Список альбомов, отсортированный по убыванию среднего рейтинга его треков (0, если треков нет)
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparingDouble(
                o -> getRatingsStream((Album) o).average().orElse(0)).reversed())
                .collect(Collectors.toList());
    }
    ...
    public static <R> Stream<R> filterIsInstance(Stream<?> s, Class<R> clazz) {
        return s.filter(o -> clazz.isAssignableFrom(o.getClass())).map(o -> (R) o);
    }
}